...
const Example = ({ authEnabled }: AppContextProps) => {
    ...
    const [accountStatusSmallScreen, setAccountStatusSmallScreen] = useState<AccountStatus>(defaultProps.accountStatus)
    const [accountStatusLargeScreen, setAccountStatusLargeScreen] = useState<AccountStatus>(defaultProps.accountStatus)

    const [chainStatusSmallScreen, setChainStatusSmallScreen] = useState<ChainStatus>(
        defaultProps.chainStatus.smallScreen
    )
    const [chainStatusLargeScreen, setChainStatusLargeScreen] = useState<ChainStatus>(
        defaultProps.chainStatus.largeScreen
    )

    const [showBalanceSmallScreen, setShowBalanceSmallScreen] = useState<boolean>(defaultProps.showBalance.smallScreen)
    const [showBalanceLargeScreen, setShowBalanceLargeScreen] = useState<boolean>(defaultProps.showBalance.largeScreen)

    const account = useAccount()
    const { chain: activeChain } = account

    const { data: transactionData, error: transactionError, sendTransaction } = useSendTransaction()

    const { data: signingData, error: signingError, signMessage } = useSignMessage()

    const { data: typedData, error: typedError, signTypedData } = useSignTypedData()
                ...
                <>
                    <div>
                        ...
                        <div style={{ display: "flex", gap: 12, paddingBottom: 12 }}>
                            <button disabled={!openConnectModal} onClick={openConnectModal} type="button">
                                {connectModalOpen ? "Connect modal opened" : "Open connect modal"}
                            </button>
                            <button disabled={!openChainModal} onClick={openChainModal} type="button">
                                {chainModalOpen ? "Chain modal opened" : "Open chain modal"}
                            </button>
                            <button disabled={!openAccountModal} onClick={openAccountModal} type="button">
                    ...
                    <div style={{ fontFamily: "sans-serif" }}>
                        <h3>Example Actions {!connected && <span>(not connected)</span>}</h3>
                        <div style={{ display: "flex", gap: 12, paddingBottom: 12 }}>
                            <button
                        ...
                        <div>
                            ...
                            {signingData && <div style={{ wordBreak: "break-all" }}>Data Signature: {signingData}</div>}
                            {signingError && <div>Error signing message</div>}
                            {typedData && (
                                <div style={{ wordBreak: "break-all" }}>Typed Data Signature: {typedData}</div>
                            )}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select
                                            id="accountStatus"
                                            onChange={(event) =>
                                                setAccountStatusSmallScreen(event.currentTarget.value as AccountStatus)
                                            }
                                    ...
                                    <td>
                                        <select
                                            id="accountStatus"
                                            onChange={(event) =>
                                                setAccountStatusLargeScreen(event.currentTarget.value as AccountStatus)
                                            }
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select
                                            id="chainStatus"
                                            onChange={(event) =>
                                                setChainStatusSmallScreen(event.currentTarget.value as ChainStatus)
                                            }
                                    ...
                                    <td>
                                        <select
                                            id="chainStatus"
                                            onChange={(event) =>
                                                setChainStatusLargeScreen(event.currentTarget.value as ChainStatus)
                                            }
    ...
    return (
        ...
        >
            ...
            <div style={{ display: "flex", gap: 12 }}>
                ...
                <input id="txHash" onChange={(e) => setHash(e.currentTarget.value)} type="text" value={hash} />
            </div>
            <div style={{ display: "flex", gap: 12 }}>
